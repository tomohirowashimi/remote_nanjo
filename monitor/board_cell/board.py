...
class Board():
    def __init__(self):
        self.line_name = "12/20"     # // *** line uniqe
        product_param = 2     # // *** line uniqe
        self.pin = 13     # // *** line uniqe
        ...
        self.font2 = 18
        self.font2_small = int(self.font2 * 0.75)
        self.pad_x = 10
        self.pad_y = 10
        ...
        file_product_day = '/home/pi/Documents/board/product_12_day.csv'
        file_product_night = '/home/pi/Documents/board/product_12_night.csv'
#        self.file_progress = '/var/tmp/progress_' + line + '.pkl'
        self.file_all_indata = '/home/pi/Documents/board/indata_all_12.pkl'
...
#        self.list_progress = ["", "", ""]        
    ...
    def gui_set(self):
        ...
        frame_h.pack()
        title = tk.Label(frame_h, text=self.line_name + "　登録画面", font=('', self.font2))
        title.pack(padx=self.pad_x, pady=self.pad_y)

        frame_b = tk.Frame(self.app)
        frame_b.pack()
        label_1 = tk.Label(frame_b, text="枚数", font=('', self.font2))
        label_1.pack(side=tk.LEFT, padx=self.pad_x, pady=self.pad_y)
#        label_1.grid(column=0, row=0, padx=self.pad_x, pady=self.pad_y)
        self.entry_1 = tk.Entry(frame_b, font=('', self.font2))
        self.entry_1.insert(tk.END, self.initial_qua)
        self.entry_1.pack(side=tk.RIGHT, padx=self.pad_x, pady=self.pad_y)
#        self.entry_1.grid(column=0, row=1, padx=self.pad_x, pady=self.pad_y)        

        frame_f = tk.Frame(self.app)
        frame_f.pack()
        label_a = tk.Label(frame_f, text="注）登録後は放置！初回入力信号が入ると自動で画面が変わります。", font=('', self.font2_small))
        label_b = tk.Label(frame_f, text="注）終了ボタンを押すとアプリケーションが終了します！", font=('', self.font2_small), fg=self.fg_labelDiff)
        button_end = tk.Button(frame_f, text="終了", command=self.gui_destroy_app, font=('', self.font2_small))
...
#        self.list_progress = [self.diffCount, averageCycle, finalTime]
    ...
    def gui_monitor(self):
        ...
        sv_label2.grid(column=1, row=2, sticky=(tk.NSEW), padx=self.pad_x, pady=self.pad_y)
        sv_label5.grid(column=3, row=1, sticky=(tk.NSEW), padx=self.pad_x, pady=self.pad_y)
        sv_label6.grid(column=3, row=2, sticky=(tk.NSEW), padx=self.pad_x, pady=self.pad_y)
